...
var firstPoint = new google.maps.LatLng($('#mark-0').attr('data-lat'), $('#mark-0').attr('data-long'));
  ...
  google.maps.event.addListener(marker[i], 'mouseup', function() {
    ...
    var longitude = this.position.lng();
	  	$('#mark-'+i).children('#lat-input').val(latitude);
	  	$('#mark-'+i).children('#long-input').val(longitude);
...
//https://developers.google.com/maps/documentation/javascript/examples/directions-simple
  ...
  google.maps.event.addListener(autocomplete, 'place_changed', function() {
	...
	$('#mark-0').children('#lat-input').val(autocompleteLat);
  	$('#mark-0').children('#long-input').val(autocompleteLong);
